...
**Live:** [https://relaxed-weasel-mautic.cloud.nexlayer.ai](https://relaxed-weasel-mautic.cloud.nexlayer.ai)  

**Runtime:**  · **Port:** auto-detected · **Deploy branch:** 7.x

---

## How this deployment works

**mautic** is deployed on [Nexlayer](https://nexlayer.ai) — a container-native
platform where every push to `7.x` triggers a fully automated build-and-deploy
...
application:
  ...
  pods:
  - name: app
    image: mirror.gcr.io/mautic/mautic:5-apache
    path: /
    servicePorts:
    - 80
    vars:
      MAUTIC_DB_HOST: mysql.pod
...
Triggers on:
- **Push** to `7.x` → production redeploy
...
> `<podName>.pod:<port>` syntax. CI/CD runs on push to `7.x`."*
